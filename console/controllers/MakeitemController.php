...
class MakeitemController extends Controller
{
    ...
    /**
     * save to cache before run
     */
    protected function saveToCache()
    {
        $item = AuthItem::find()->asArray()->all();
        $child = AuthItemChild::find()->asArray()->all();
        $cache = Yii::$app->cache;
        if (false === $cache->set('item', $item)) {
            exit('save item cache error');
        }
        if (false === $cache->set('child', $child)) {
            exit('save child cache error');
        }
        // $cache->getOrSet('item');
    }

    /**
     * run 前先从数据库中删除 item 项, child 是中的字段外键来自item表所以同时被删除
